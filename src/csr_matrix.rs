...
use crate::{dok_matrix::DokMatrix, is_increasing, Matrix};
...
/*
// rayon par_quicksort, but also applying the same permutation sorting v to w
fn par_quicksort<T, U, F>(v: &mut [T], w: &mut [U], is_less: F)
where
    T: Send,
    F: Fn(&T, &T) -> bool + Sync,
{
...
fn insertion_sort<T: Copy, U, F: Fn(&T, &T) -> bool>(v: &mut [T], w: &mut [U], is_less: &F) {
    for i in 1..v.len() {
        shift_tail(&mut v[..=i], &mut w[..=i], is_less);
    }
}

fn shift_tail<T: Copy, U, F: Fn(&T, &T) -> bool>(v: &mut [T], w: &mut [U], is_less: &F) {
    use std::ptr;
    let len = v.len();
...
#[cold]
fn heapsort<T: Copy, U, F: Fn(&T, &T) -> bool>(v: &mut [T], w: &mut [U], is_less: &F) {
    // This binary heap respects the invariant `parent >= child`.
    let sift_down = |v: &mut [T], w: &mut [U], mut node| {
        loop {
            // Children of `node`:
            let left = 2 * node + 1;
            let right = 2 * node + 2;
